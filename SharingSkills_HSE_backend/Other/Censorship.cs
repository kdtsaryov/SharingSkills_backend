...
{
    /// <summary>
    /// Класс для автоматической цензуры нецензцрной лексики
    /// </summary>
    public class Censorship
    {
        ...
        /// </summary>
        private static Dictionary<char, List<char>> d = new Dictionary<char, List<char>>() {
            { 'а', new List<char> { 'а', 'a', '@' } },
        ...
        /// <summary>
        /// Непосредственно зацензуривание
        /// </summary>
        /// <param name="phrase">Фраза</param>
        public static string DoCensorship(string phrase)
        {
            ...
            foreach (string w in words)
            {
                for (int i = 0; i < newphrase.Length - w.Length + 1; i++)
                {
                    ...
                    {
                        // Заменяем отрезок на символы цензуры
                        string fr = phrase.Substring(i, w.Length);
                        phrase = phrase.Replace(fr, "***");
                        newphrase = newphrase.Replace(fragment, "***");
                    }
                }
            }
            return phrase;
